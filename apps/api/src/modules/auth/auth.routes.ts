...
/**
 * §8 — a SuperAdmin cannot sign in until 2FA is set up, which leaves a
 * chicken-and-egg problem: enrolling needs a session, and a session needs
 * enrolment. This route breaks it by taking the password again and issuing only
 * a secret — never a token.
 */
authRouter.post(
  '/2fa/bootstrap',
  loginLimiter,
  validateBody(loginSchema.omit({ totpCode: true })),
  asyncRoute(async (req, res) => {
    await assertNotLocked(req.body.phone, req.ip)
    const user = await User.findOne({ phone: req.body.phone, deletedAt: null }).select(
      '+passwordHash',
    )

    if (!user || !(await verifyPassword(user.passwordHash, req.body.password))) {
      if (!user) await burnPasswordTime()
      await recordFailure(req.body.phone, req.ip)
    ...
    if (!isSuperadmin(user) || user.twoFactor?.enabled) {
      // Everyone else enrols from inside a normal session.
      throw ApiError.forbidden('Use /auth/2fa/enable from a signed-in session')
    }
...
authRouter.post(
  ...
  loginLimiter,
  validateBody(loginSchema.extend({ totpCode: totpVerifySchema.shape.code })),
  asyncRoute(async (req, res) => {
    const user = await User.findOne({ phone: req.body.phone, deletedAt: null }).select(
      '+passwordHash',
    )
    ...
    await confirmTwoFactor(user.id, req.body.totpCode, req)
    res.json({ data: { enabled: true } })
  }),
)
