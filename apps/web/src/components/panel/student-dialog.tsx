...
function StudentActions({ student, onDone }: { student: PanelStudent; onDone: () => void }) {
  ...
  const freeze = useMutation<undefined, unknown>(
    `/students/${student._id}/${student.status === 'frozen' ? 'unfreeze' : 'freeze'}`,
  )
  ...
  return (
    <>
      ...
      <Action
        label={t(student.status === 'frozen' ? 'unfreeze' : 'freeze')}
        Icon={student.status === 'frozen' ? Play : Snowflake}
